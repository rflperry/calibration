    ...
    def __init__(self,
        ...
        Args:
          ...
          reduction: (Optional) Type of `tf.keras.losses.Reduction` to apply to
            ...
            `tf.distribute.Strategy`, outside of built-in training loops (i.e.
            `tf.keras`, `compile`, `fit`), `AUTO` or `SUM_OVER_BATCH_SIZE`
...
def _validate_contrastive_loss_inputs(features, labels, contrast_mode,
    ...
    if features.shape.rank < 2:
        raise ValueError(
            ...
            '>= 2 with shape [batch_size, num_views] + `feature shape.`')
...
def _cap_positives_mask(untiled_mask, diagonal_mask, num_views, positives_cap):
    r"""Cap positives in the provided untiled_mask.
        ...
        views.
    Args:
      untiled_mask: Tensor of shape [batch_size, batch_size] that has
      ...
      diagonal_mask: Tensor with the same shape as `untiled_mask`. When
        batch_size == batch_size this is just an identity matrix.
        Otherwise, it is an identity matrix of size `batch_size` that is
    ...
    # [untiled_positives_per_anchor * batch_size, 2]. Each row represents
    # the 2D index in a
    # [batch_size * num_anchor_views, batch_size * num_views] size
...
def _create_tiled_masks(untiled_mask, diagonal_mask, num_views,
    ...
    Tiles `untiled_mask`, which has shape [batch_size, batch_size]
    ...
    [b1v1, b2v1, b3v1, b1v2, b2v2, b3v2]:
      positives_mask: Entry (row = i, col = j) is 1 if
        untiled_mask[i % batch_size, j % batch_size] == 1 and
        i // batch_size != j // batch_size. This results in a mask
    ...
    Args:
      untiled_mask: Tensor of shape [batch_size, batch_size], where
        batch_size <= batch_size, that has entry (r, c) == 1 if
      ...
      diagonal_mask: Tensor with the same shape as `untiled_mask`. When
        batch_size == batch_size this is just an identity matrix.
        Otherwise, it is a slice of a [batch_size, batch_size]
    ...
    """
    batch_size = tf.shape(untiled_mask)[1]
    # Generate |all_but_diagonal_mask|, a tensor of shape
    # [batch_size * num_anchor_views, batch_size * num_views] where
    ...
    # the case where batch_size == batch_size and
    ...
    for i in range(num_anchor_views):
        tiled_labels.append(
            labels + tf.cast(batch_size, labels.dtype) * i)
    tiled_labels = tf.concat(tiled_labels, axis=0)
    tiled_diagonal_mask = tf.one_hot(
        tiled_labels, batch_size * num_views)
    all_but_diagonal_mask = 1. - tiled_diagonal_mask

    # Construct |negatives_mask| and |uncapped_positives_mask|, both tensors of
    # shape [batch_size * num_anchor_views, batch_size * num_views].
    ...
    uncapped_positives_mask = tf.tile(
    ...
    if positives_cap > -1:
        ...
        # [batch_size * num_anchor_views, batch_size * num_views].
        # Entry (r,c) is 1 iff
        # untiled_mask[r % batch_size, c % batch_size] == 1 *and*
        # r != c.
        # Else it is zero.
    ...
    # Zero the diagonal.
    positives_mask = positives_mask * all_but_diagonal_mask
...
def contrastive_loss(features,
    ...
    # If no data augmentation (i.e. singleview), reshape to multiview
    if features.shape.rank == 2:
        features = tf.expand_dims(features, 1, 'shaped_features')
    labels = tf.convert_to_tensor(labels) if labels is not None else None

    batch_size, num_views = _validate_contrastive_loss_inputs(
    ...
    # [batch_size, num_views, num_features].
    ...
    # Generate the [batch_size, batch_size] slice of the
    # [batch_size, batch_size] identity matrix that corresponds to
    # the current replica.
    diagonal_mask = tf.one_hot(tf.range(batch_size), batch_size)

    # Generate `mask` with shape [batch_size, batch_size] that
    ...
    if labels is None:
        ...
        mask = tf.linalg.matmul(labels, labels, transpose_b=True)
    mask = tf.ensure_shape(mask, [batch_size, batch_size])

    # To streamline the subsequent TF, the first two dimensions of
    # `features` (i.e., batch_size and num_views) should be
    # transposed and then flattened. The result has shape
    # [num_views * batch_size, num_features], and its first dimension
    # elements are grouped by view, not by sample.
    all_features = tf.reshape(
    ...
    if contrast_mode == 'one':
        ...
        num_anchor_views = 1
    else:  # contrast_mode == 'all''
        # Reshape features to match how  is reshaped above.
    ...
    # [batch_size * num_anchor_views, batch_size * num_views]. To
    ...
    logits = tf.linalg.matmul(
        anchor_features, all_features, transpose_b=True)
    ...
    else:  # summation_location == 'inside'
    ...
    if scale_by_temperature:
        loss *= temperature
    loss = tf.reshape(loss, [num_anchor_views, batch_size])
    ...
    if num_views != 1:
        ...
        num_valid_views_per_sample = (
            tf.reshape(num_positives_per_row, [1, batch_size]))
        loss = tf.squeeze(tf.math.divide_no_nan(
            loss, num_valid_views_per_sample))
